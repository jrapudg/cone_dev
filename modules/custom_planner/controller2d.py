...
from modules.custom_planner import cutils
import numpy as np
...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        self.vars.create_var('kp', 0.5)              #0.5  #2.0      #0.6 #1.0
        self.vars.create_var('ki', 0.4)              #0.4  #0.05      #0.08 #0.05
        self.vars.create_var('integrator_min', 0.0)
        self.vars.create_var('integrator_max', 10.0)
        self.vars.create_var('kd', 0.2)              #0.4  #0.3      #0.08 #0.3
        self.vars.create_var('kp_heading', 2.0)
        ...
        if self._start_control_loop:
            ...
            steer_output = heading_error + \
                    np.arctan(self.vars.kp_heading * \
                              crosstrack_sign * \
                              crosstrack_error / \
                              (v + self.vars.k_speed_crosstrack + 1e-6))
