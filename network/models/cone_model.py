...
from models.context_encoders import MapEncoderPts
...
class ConstructEgo(nn.Module):
    '''
    Construc Pred Class.
    '''
    def __init__(self, d_k=128, _M=5, c=5, T=30, L_enc=1, dropout=0.0, k_attr=2, map_attr=3,
                 num_heads=16, L_dec=1, tx_hidden_size=384, construction_attention=True, sequence_attention=True, baseline=False):
        super(ConstructEgo, self).__init__()
        ...
        self.construction_attention = construction_attention
        self.sequence_attention = sequence_attention
        self.baseline = baseline
        print("CONFIGS")
        print("baseline: {}".format(self.baseline))
        print("construction_attention: {}".format(self.construction_attention))
        print("sequence_attention: {}".format(self.sequence_attention))
        # INPUT ENCODERS
        self.agents_dynamic_encoder = nn.Sequential(init_(nn.Linear(k_attr, d_k)))
        print("k_attr: {}".format(self.k_attr))
        print("map_attr: {}".format(self.map_attr))
        # ============================== AutoBot-Ego ENCODER ==============================
    ...
    def forward(self, ego_in, roads, constructions):
        '''
        ...
        ego_tensor, env_masks = self.process_observations(ego_in)
        ...
        #agents_tensor_2 = ego_tensor.unsqueeze(2)
        #print("Seven: {}".format(agents_tensor.shape))
        
        agents_tensor = ego_tensor
        #print("agents_emb: {}".format(agents_tensor.shape))
        
        #print("One: {}".format(agents_tensor_2.shape))
        #agents_emb = self.agents_dynamic_encoder(agents_tensor).permute(1, 0, 2, 3)
        agents_emb = self.agents_dynamic_encoder(agents_tensor).permute(1, 0, 2)
        
        #print("encoded_agents_emb: {}".format(agents_emb.shape))
        # Process through AutoBot's encoder

        ego_soctemp_emb = agents_emb
        ...
        orig_cons_features, orig_cons_segs_masks = self.map_encoder(constructions, ego_soctemp_emb)
        
        #print("Roads: {}".format(roads.shape))
        #print("Construction: {}".format(constructions.shape))
        obs_features = torch.cat((roads, constructions.repeat(1, 1, S, 1)), dim=1)
        orig_obs_features, obs_features_masks = self.map_encoder(obs_features, ego_soctemp_emb)
        #print("Combined: {}".format(obs_features.shape))
        ...
        construction_features = orig_cons_features.unsqueeze(2).repeat(1, 1, self.c, 1).view(-1, B*self.c, self.d_k)
        construction_segs_masks = orig_cons_segs_masks.unsqueeze(1).repeat(1, self.c, 1).view(B*self.c, -1)

        combined_features = orig_obs_features.unsqueeze(2).repeat(1, 1, self.c, 1).view(-1, B*self.c, self.d_k)
        combined_features_mask = obs_features_masks.unsqueeze(1).repeat(1, self.c, 1).view(B*self.c, -1)
        ...
        #print("After squeeze const: {}".format(construction_features.shape))
        #print("After squeeze mask: {}".format(construction_segs_masks.shape))
                ...
                for d in range(self.L_dec):
                    if d == 1:
                        ego_dec_emb_map = self.map_attn_layers(query=out_seq, 
                                                               ...
                                                               key_padding_mask=combined_features_mask)[0]
        ...
        if self.baseline:
            ...
            if self.sequence_attention:
                ...
                mode_params_emb = self.mode_map_attn(query=mode_params_emb, key=orig_map_features, value=orig_map_features,
                                                key_padding_mask=orig_road_segs_masks)[0] + mode_params_emb"""
                mode_params_emb = self.mode_map_attn(query=mode_params_emb, key=orig_obs_features, value=orig_obs_features,
                                                        key_padding_mask=obs_features_masks)[0] + mode_params_emb
            else:
                mode_params_emb = self.mode_map_attn(query=mode_params_emb, key=orig_obs_features, value=orig_obs_features,
                                                        key_padding_mask=obs_features_masks)[0] + mode_params_emb
