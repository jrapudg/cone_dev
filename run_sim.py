...
from modules.construction import spawn_construction_zone
...
def simulate(world, save_gift=False):
    ...
    ## Spawn Ego Vehicle
    available_vehicles = ['vehicle.audi.a2', 'vehicle.audi.etron', 'vehicle.citroen.c3',
                          'vehicle.jeep.wrangler_rubicon', 'vehicle.volkswagen.t2_2021',
    ...
    number_cones = random.randint(3, 15)
    cone_line = {'elements':['constructioncone']*number_cones,
                'offsets':[(0,0)]*number_cones,
                'rotations':[0]*number_cones,
                'type':[0]*4}

    number_big_cones = random.randint(3, 15)
    construction_1 = {'elements':['trafficwarning'] + ["trafficcone01"]*(number_big_cones),
                    'offsets':[(0,0)]*(number_big_cones+1),
                    'rotations':[90] + [0]*(number_big_cones),
                    'type':[0]*(number_big_cones+1)}

    spacing = 0.5
    cone_diamond = {'elements':['constructioncone']*4,
                    'offsets':[(0,0), (2*spacing, spacing), (2*spacing, -spacing), (4*spacing,0)],
                    'rotations':[0]*4,
                    'type':[1]*4}

    scenarios = [cone_line, construction_1, cone_diamond]
    ...
    ## Spawn construction objects
    offset = random.uniform(-1, 1)
    modified_waypoints, rotations = add_lateral_offset(waypoints, offset)

    sceario_index = random.randint(0, 2)
    spawn_construction_zone(bp_lib, world, scenarios[sceario_index], modified_waypoints, DIST_TO_FIRST_OBJ)
    ...
    for frame in range(2500):
        ...
        if frame % LP_FREQUENCY_DIVISOR == 0:
            ...
            #image_sem_front = sem_front_queue.get()
            #img_sem_front = np.reshape(np.copy(image_sem_front.raw_data), 
            #                           (image_sem_front.height, image_sem_front.width, 4)) 
            #img_semantic_mask = process_semantic_image(image_sem_front)
            #img_semantic_mask = get_drivable_surface(img_semantic_mask, include_construction=True)
        
            #path_np = np.array(paths)
            #path_np = np.array(path_np[:,:2,:]).transpose(0,2,1)
            #probs = [] 
            #for i in range(path_np.shape[0]): 
            #    prob = check_endpoint_in_mask(path_np[i], img_semantic_mask.copy(), K_sem_front, world_2_camera_sem_front)        
            #    probs.append(prob)
                
            #new_collision = [a and b for a, b in zip(probs, collision_check_array)] 
            #print(new_collision)
            #print(probs)
            #print(collision_check_array)
            
            #if any(new_collision):
            #    print("New collision")
            #    #  # Compute the best local path.
            #    best_index = lp._collision_checker.select_best_path_index(paths, new_collision, bp._goal_state)
            #else:
            #    print("Normal")
            best_index = lp._collision_checker.select_best_path_index(paths, collision_check_array, bp._goal_state)
            #print("Path taken {}".format(best_index))
            ...
            if local_waypoints != None:
                #point = carla.Location(x=local_waypoints[0][0], y=local_waypoints[0][1], z=local_waypoints[0][2])
                #w = carla_map.get_waypoint(point)
                #print("Path {}: {}".format(local_waypoints[0], w.lane_id))
    ...
    #lane_centers_pts_np = generate_map(current_x, current_y, area_range=80, distance_spread=1.0)
    ...
    #lane_centers_pts_np = substract_ego_location(padded_waypoints, ego_points_loc)
    #lane_centers_pts_np = rotation_yaw_3d(lane_centers_pts_np, ego_yaw-90)
    ...
    #ego_points_np = rotation_yaw(ego_points_np, ego_yaw-90)
    #waypoints_pts_np = rotation_yaw_3d(waypoints_pts_np, ego_yaw-90)
    #static_points_np = rotation_yaw_3d(static_points_np, ego_yaw-90)
    #trajectory_control_np = rotation_yaw(trajectory_control_np, ego_yaw-90)
...
PATH_OFFSET            = 1.0            # m
CIRCLE_OFFSETS         = [2.0, 0.0, -2.0] # m
CIRCLE_RADII           = [1.2, 1.2, 1.2]  # m
TIME_GAP               = 1.0              # s
...
if __name__ == '__main__':
    ...
    for _ in range(5):
        TARGET_SPEED = random.uniform(2.6, 5.0)
