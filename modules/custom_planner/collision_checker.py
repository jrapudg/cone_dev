...
class CollisionChecker:
    def __init__(self, circle_offsets, circle_radii, weight, carla_map):
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                #for i in range(50):
                    #your_location = carla.Location(x=paths[i][0][25], y=paths[i][1][25], z=0)
                #closest_waypoint = self.carla_map.get_waypoint(your_location, project_to_road=True, lane_type=carla.LaneType.Driving)
                    
                #print(np.array(paths).shape)
                your_location = carla.Location(x=paths[i][0][-1], y=paths[i][1][-1], z=0)
                closest_waypoint = self.carla_map.get_waypoint(your_location, project_to_road=True, lane_type=carla.LaneType.Driving)
                distance = your_location.distance(closest_waypoint.transform.location)
